...
sel_participant_roomtypes = [(rt.id, rt.detailed_description)
                             for rt in roomtypes.values() if rt.for_participants]
sel_guest_roomtypes = [(rt.id, rt.detailed_description)
                       for rt in roomtypes.values() if rt.for_guests]

participants = session.query(Participant).all()
sel_all_participants = [(str(prt.id), prt.fullnameLF())
                        for prt in sorted(participants, key=lambda prt: prt.fullnameLF())]
    ...
    def _check(form, guest_roomtype_field):
        ...
        if grf == NO_GUEST and form[guestname_field_name].data:
            raise ValidationError(
                "You specified a name for a guest, but chose 'no guest' as the room type.")
